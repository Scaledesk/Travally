<?php 
    ...
    add_filter('posts_join' , array($tour , '_get_join_query'));
    add_filter('posts_where' , array($tour , '_get_where_query_tab_location'));

    query_posts($query);    
    
    remove_filter('posts_join' , array($tour , '_get_join_query'));
    remove_filter('posts_where' , array($tour , '_get_where_query_tab_location'));
    if (have_posts()){
        while(have_posts()){
            the_post();

<div class="row">
    <?php if($map_position == 'left'): ?>
        <div class="col-md-6 st_gmap">
            <div id="gmap_wrapper" class="st_list_half_map">
                <div class="content_map" style="height: <?php echo esc_html( $height ) ?>px">
                    <div id="list_half_map" class="gmap3" data-fitbounds="<?php echo esc_html($fit_bounds)?>" style="height: <?php echo esc_html( $height ) ?>px; width: 100%"></div>
                </div>
    ...
    <?php endif; ?>
    <div class="col-md-6 div_half_map" style="height: <?php echo esc_html( $height ) ?>px;">
        ...
        <?php endif ?>
        <h2><?php printf(__("Advanced Search <span class='count_advan_saerch'>(%s)</span>",ST_TEXTDOMAIN),count($data_map)) ?></h2>
        ...
        $('#hotel_search_half_map .btn_search').click(function () {
            ...
            var options = {
                ...
                success: function (data) {
                    ...
                    $('.data_list_hafl_map').html('');
                    var count = 0;
                    for (var key in data.data_map) {
                        var tmp_data = data.data_map[key];
                        console.log(tmp_data);
                        $('.data_list_hafl_map').append('<div class="col-md-6">' + tmp_data.content_adv_html + '</div>');
                        count++;
                    }
                    $('.count_advan_saerch').html("("+count+")");
                    if(count == 0){
                        $('.data_list_hafl_map').append('<div class="alert alert-warning"> <button data-dismiss="alert" type="button" class="close"><span aria-hidden="true">×</span> </button> <p class="text-small">'+st_list_map_params.text_no_result+'</p> </div>');
                    }

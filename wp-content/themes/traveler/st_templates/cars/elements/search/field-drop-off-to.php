...
<div class="form-group form-group-<?php echo esc_attr($field_size)?> form-group-icon-left">
    ...
    <div class="st-select-wrapper">
        <input autocomplete="off" type="text" name="drop-off" value="<?php echo $location_name; ?>" class="form-control st-location-name <?php echo esc_attr($is_required); ?>" placeholder="<?php if($placeholder) echo $placeholder; ?>">
        <select id="field-st-dropoff-to" name="location_id_drop_off" <?php echo esc_attr($is_required) ?> class="st-location-id st-hidden " placeholder="<?php if($placeholder) echo $placeholder; ?>" tabindex="-1">
            <option value=""></option>
            <?php 

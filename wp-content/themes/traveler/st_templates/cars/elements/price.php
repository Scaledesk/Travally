...
<div class="booking-item-price-calc">
            ...
            if(!empty($list)){
                foreach($list as $k=>$v){
                    ...
                    echo '<div class="checkbox">
                            <label>
                                <input class="i-check equipment" data-price-max="'.$price_max.'" data-price-unit="'.$price_unit.'" data-title="'.$v['title'].'" data-price="'.$v['cars_equipment_list_price'].'" type="checkbox" />'.$v['title'].'
            ...
            <ul class="list">
                ...
                <li>
                    <p> <?php echo st_get_language('car_price_per').' '.ucfirst(STCars::get_price_unit('label')); ?>
                        <span><?php echo TravelHelper::format_money($price_origin) ?></span>
                    </p>
                </li>
                <?php }else{ ?>
                    <li>
                ...
                <li><p>
                    ...
                    <?php }else{ ?>
                        <small><?php echo esc_attr($time) ?> <?php if($time > 1) echo STCars::get_price_unit('plural'); else STCars::get_price_unit('label'); ?> </small>
                    <?php } ?>
                </li>
            <?php endif; ?>
                <li><p>
    ...
    <input type="hidden" name="location_id_pick_up" class="" value="<?php echo $location_id_pick_up; ?>">
    <input type="hidden" name="location_id_drop_off" class="" value="<?php echo $location_id_drop_off; ?>">

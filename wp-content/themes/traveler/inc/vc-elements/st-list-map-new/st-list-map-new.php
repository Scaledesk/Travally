...
if(!class_exists('st_list_map')){
    class st_list_map{
        static function _get_query_where($where){
            ...
            if(!TravelHelper::checkTableDuplicate($st_type)) return $where;
            global $wpdb;

            $location_field = 'id_location';
            if($st_type == 'st_rental') $location_field = 'location_id';
        ...
        static function _get_query_join($join){
            $st_type = $_SESSION['el_st_type'];
            if(!TravelHelper::checkTableDuplicate($st_type)) return $join;
            global $wpdb;

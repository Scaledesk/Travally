...
if(!class_exists('NewOrderData')){
	class NewOrderData{
		...
		public function __construct(){
			add_action('st_save_order_item_meta',array(&$this,'_save_data'),10,3);
			add_action('st_booking_change_status',array(&$this,'_st_booking_change_status'),10,3);
			add_action('st_traveler_do_upgrade_table', array(&$this, '_action_check_upgrade_order'));
			add_action('after_setup_theme',array(&$this,'_check_table_order'), 10);
			add_action('after_setup_theme',array(&$this,'_check_upgrade_order'), 50);
		}
		...
		public function _check_table_order(){
			...
			$column = array(
	            ...
	            'room_num_search' => array(
	            	'type' => 'varchar',
	            	'length' => 255
	            ),
	            'check_in_timestamp' => array(
	            	'type' => 'varchar',
	            	'length' => 255
	            ),
	            'check_out_timestamp' => array(
	            	'type' => 'varchar',
	            	'length' => 255
	            ),
	            'status' => array(
		...
		public function _save_data($data = array(), $order_item_id = false, $type = 'normal_booking'){
			if(is_array($data) && count($data)){
                ...
                $total_order = isset($data['total_price']) ? $data['total_price'] : 0;

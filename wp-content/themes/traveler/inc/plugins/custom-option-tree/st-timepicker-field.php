...
class ST_TimePicker_Field
{
    ...
    function add_scripts(){
        wp_enqueue_style('st_timepicker.css', $this->url.'/css/st_timepicker.css');
    	wp_enqueue_script('st_timepicker',$this->url.'/js/st_timepicker.js',array('jquery'),null,true);
    }
...
function ot_type_st_timepicker($args = array())
{
    ...
    echo balanceTags($has_desc ? '<div class="description">' . htmlspecialchars_decode( $field_desc ) . '</div>' : '');
        ...
        echo '<input class="widefat option-tree-ui-input st_timepicker" value="'.esc_attr($field_std).'" placeholder="hh:mm tt" name="'.esc_attr( $field_name ).'" id="'.esc_attr( $field_id ).'">';
        echo '</div>';
        echo '</div>';
}
endif;
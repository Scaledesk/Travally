...
class STCustomselectpost
{
    ...
    function add_scripts()
    {
        ...
        $lang=get_locale();
        $lang_file=$this->dir.'/js/select2-3.5.2/select2_locale_'.$lang.'.js';
        if(file_exists($lang_file))
        {
            wp_enqueue_script('select2-lang',$this->url.'/js/select2-3.5.2/select2_locale_'.$lang.'.js',array('jquery','select2'),null,true);
        }else{
            $locale_array=explode('_',$lang);
            if(!empty($locale_array) and $locale_array[0] and $locale_array[0]!='en'){
                $locale=$locale_array[0];

                $lang_file=$this->dir.'/js/select2-3.5.2/select2_locale_'.$locale.'.js';

                if(file_exists($lang_file))
                wp_enqueue_script('select2-lang',$this->url.'/js/select2-3.5.2/select2_locale_'.$locale.'.js',array('jquery','select2'),null,true);
            }
        }
        wp_enqueue_style('select2_css',$this->url.'/js/select2-3.5.2/select2.css');
        wp_enqueue_script('st_post_select',$this->url.'/js/st_post_select.js',array('jquery'),null,true);
    }
